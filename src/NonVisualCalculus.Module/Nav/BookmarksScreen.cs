...
namespace NonVisualCalculus.Module.Nav
{
    ...
    internal sealed class BookmarksScreen : ModOverlay
    {
        ...
        /// "detour required" when the walk there goes through a self-opening gate first, else the sealing
        /// rule when known ("requires a flashlight") or "can't reach" when no path
        ...
        internal string RowLine(Bookmark mark)
        {
            ...
            WalkRoute route = _world.Reach(mark.Position, out string reason);
            return BookmarkAnnouncer.Compose(mark.Name, meters, route, reason: reason);
        }
        ...
        internal string RowLine(PresetBookmark preset)
        {
            ...
            WalkRoute route = _world.Reach(preset.Position, out string reason);
            return BookmarkAnnouncer.Compose(preset.Name(), meters, route, preset: true, reason: reason);
        }
